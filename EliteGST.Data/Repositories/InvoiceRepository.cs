...
namespace EliteGST.Data.Repositories
{
    public class InvoiceRepository : BaseRepository<Invoice>, IInvoiceRepository
    {
        ...
        {
            var cols = Table + ".*";
            var sql = string.Format("SELECT {0}, parties.CompanyName AS Customer, parties.GSTIN AS GSTIN FROM {1} INNER JOIN parties ON parties.Id={1}.BillingId WHERE parties.CompanyName LIKE CONCAT('%', @name, '%')", cols, Table);
